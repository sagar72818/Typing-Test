# Typing Speed Test Game in PYTHON
from tkinter import *
...
def game():
    ...
    def check_result():
        ...
        if wpm > line:
            ...
            result = "Congrats ! You hit a new highscore of " + str(wpm) + " WPM"
            messagebox.showinfo("Score", result)
        else:
            result = "Your score is :" + str(wpm) + " WPM\nHigh Score : " + str(line) + " WPM"
            messagebox.showinfo("Score", result)
...
b1 = Button(root,text="Go!",width=12,bg="yellow",font="times 20",command=game)
b1.place(x=150,y=120)

hs_text = Label(root,text="Highscore : ",width=10,bg="#03fcf8",font="times 35")
hs_text.place(x=30,y=240)

hs = int(hs_file.readline())
hs_val = Label(root,text=str(hs)+" WPM",width=8,fg="#03fcf8",bg="black",font="times 25")
hs_val.place(x=330,y=250)
